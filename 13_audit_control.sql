-- ====================================================
-- SCRIPT: 13_audit_control.sql
-- DESCRIPCIÓN: Sistema de control de auditoría automático
...
BEGIN
    SP_REGISTER_AUDIT(
        ...
        p_operation_type => 'INSERT',
        p_additional_info => 'Manufacturer: ' || :NEW.MANUFACTURER_NAME
    );
END;
/
...
BEGIN
    SP_REGISTER_AUDIT(
        p_table_name => 'MANUFACTURERS',
        p_operation_type => 'DELETE',
        p_additional_info => 'Manufacturer: ' || :OLD.MANUFACTURER_NAME
    );
END;
/
...
BEGIN
    SP_REGISTER_AUDIT(
        ...
        p_operation_type => 'INSERT',
        p_additional_info => 'Region: ' || :NEW.REGION_NAME
    );
END;
/
...
BEGIN
    SP_REGISTER_AUDIT(
        p_table_name => 'REGIONS',
        p_operation_type => 'DELETE',
        p_additional_info => 'Region: ' || :OLD.REGION_NAME
    );
END;
/
...
-- Triggers para FUEL_TYPES
CREATE OR REPLACE TRIGGER TRG_FUEL_TYPES_INSERT
    AFTER INSERT ON FUEL_TYPES
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('FUEL_TYPES', 'INSERT', 1, 'Fuel Type: ' || :NEW.FUEL_TYPE_NAME);
END;
/

CREATE OR REPLACE TRIGGER TRG_FUEL_TYPES_UPDATE
    AFTER UPDATE ON FUEL_TYPES
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('FUEL_TYPES', 'UPDATE', 1, 'Fuel Type: ' || :NEW.FUEL_TYPE_NAME);
END;
/

CREATE OR REPLACE TRIGGER TRG_FUEL_TYPES_DELETE
    AFTER DELETE ON FUEL_TYPES
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('FUEL_TYPES', 'DELETE', 1, 'Fuel Type: ' || :OLD.FUEL_TYPE_NAME);
END;
/
...
CREATE OR REPLACE TRIGGER TRG_TRANSMISSIONS_INSERT
    ...
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('TRANSMISSIONS', 'INSERT', 1, 'Transmission: ' || :NEW.TRANSMISSION_NAME);
END;
/
...
CREATE OR REPLACE TRIGGER TRG_TRANSMISSIONS_UPDATE
    ...
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('TRANSMISSIONS', 'UPDATE', 1, 'Transmission: ' || :NEW.TRANSMISSION_NAME);
END;
/
...
CREATE OR REPLACE TRIGGER TRG_TRANSMISSIONS_DELETE
    ...
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('TRANSMISSIONS', 'DELETE', 1, 'Transmission: ' || :OLD.TRANSMISSION_NAME);
END;
/

-- Triggers para DRIVE_TYPES
CREATE OR REPLACE TRIGGER TRG_DRIVE_TYPES_INSERT
    AFTER INSERT ON DRIVE_TYPES
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('DRIVE_TYPES', 'INSERT', 1, 'Drive Type: ' || :NEW.DRIVE_TYPE_NAME);
END;
/

CREATE OR REPLACE TRIGGER TRG_DRIVE_TYPES_UPDATE
    AFTER UPDATE ON DRIVE_TYPES
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('DRIVE_TYPES', 'UPDATE', 1, 'Drive Type: ' || :NEW.DRIVE_TYPE_NAME);
END;
/

CREATE OR REPLACE TRIGGER TRG_DRIVE_TYPES_DELETE
    AFTER DELETE ON DRIVE_TYPES
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('DRIVE_TYPES', 'DELETE', 1, 'Drive Type: ' || :OLD.DRIVE_TYPE_NAME);
END;
/

-- Triggers para VEHICLE_CLASSES
CREATE OR REPLACE TRIGGER TRG_VEHICLE_CLASSES_INSERT
    AFTER INSERT ON VEHICLE_CLASSES
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('VEHICLE_CLASSES', 'INSERT', 1, 'Class: ' || :NEW.CLASS_NAME);
END;
/

CREATE OR REPLACE TRIGGER TRG_VEHICLE_CLASSES_UPDATE
    AFTER UPDATE ON VEHICLE_CLASSES
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('VEHICLE_CLASSES', 'UPDATE', 1, 'Class: ' || :NEW.CLASS_NAME);
END;
/

CREATE OR REPLACE TRIGGER TRG_VEHICLE_CLASSES_DELETE
    AFTER DELETE ON VEHICLE_CLASSES
    FOR EACH ROW
BEGIN
    SP_REGISTER_AUDIT('VEHICLE_CLASSES', 'DELETE', 1, 'Class: ' || :OLD.CLASS_NAME);
END;
/
...
PROMPT '*** SCRIPT 13_audit_control.sql COMPLETADO ***'
